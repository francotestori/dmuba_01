...
TransformaciÃ³nes del dataframe de profiles
```{r}
...
top_users_by_retweet_count= top_users_by_retweet_count %>%
  ...
  filter(row_number()==1)
View(head(top_users_by_retweet_count))
```
...
#Ordenados de Mayor a menor por Retweet_count??
df_tweets
...
#Correlación con los perfiles de mayor impacto
...
```
#5)Correlación variables numéricas
```{r}
